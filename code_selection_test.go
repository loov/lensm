...
func TestTextSelectionAssembly(t *testing.T) {
	...
	selection := TextSelection{View: CodeViewGoAsm, Anchor: 1, Head: 0, Active: true}
	if got, want := selection.Text(code), "MOV (R2), R1\nADDQ $1, R1\n"; got != want {
		t.Fatalf("Text() = %q, want %q", got, want)
	}
	selection.View = CodeViewNativeAsm
	if got, want := selection.Text(code), "MOV (%R2), %R1\nADDQ $1, %R1\n"; got != want {
...
func TestTextSelectionSource(t *testing.T) {
	...
	selection := TextSelection{View: CodeViewSource, Anchor: 0, Head: 2, Active: true}
